...
export default function AdminProducts() {
  ...
  const handleSave = async e => {
    ...
    try {
      const payload = {
        ...form,
        pricePerStem:     Number(form.pricePerStem),
  ...
  const handleDelete = async id => {
    ...
    try { await adminAPI.deleteProduct(id); toast.success('Product deleted'); load() }
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                <td>
                  <img
                    src={p.imageUrl || 'https://images.unsplash.com/photo-1560717789-0ac7c58ac90a?w=80'}
                    alt={p.name}
                    style={{ width: 44, height: 44, objectFit: 'cover', borderRadius: 6 }}
                    onError={e => e.target.src = 'https://images.unsplash.com/photo-1560717789-0ac7c58ac90a?w=80'}
                  />
                </td>
                ...
                <td style={{
                  color: p.stockQuantity < 200 ? '#e53e3e' : 'inherit',
                  fontWeight: p.stockQuantity < 200 ? 700 : 400
                }}>{p.stockQuantity?.toLocaleString('en-IN')}</td>
                <td>
                  <span style={{
